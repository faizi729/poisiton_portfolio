...
const PnLPage: React.FC = () => {
  ...
  // ✅ Fetch realized PnL once userId is available
  useEffect(() => {
    if (!userId) return;
      ...
      <div className="p-8">
        ...
        <div>
          ...
          <table className="w-full border">
           ...
           <tbody>
  {data?.realizedTrades && data.realizedTrades.length > 0 ? (
    data.realizedTrades.map((trade) => (
      <tr key={trade.id}>
        <td className="border p-2">{trade?.symbol}</td>
        <td className="border p-2">{trade?.trade?.tradeType || "—"}</td>
        ...
        <td className="border p-2">{trade?.lot?.avgPrice ?? "—"}</td>
